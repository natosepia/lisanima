...
| 項目 | 決定 | 理由 |
|------|------|------|
| 認証フレームワーク | FastMCP内蔵OAuth 2.0 AS | mcp 1.26.0に完全実装済み。自前実装不要 |
...
Desktop App                    nginx (SSL)              lisanima (localhost:8765)
    ...
    │←─────────────────────────────│←──────────────────────────│
    │                              │                           │
    │  GET /lisanima/.well-known/oauth-authorization-server    │
    │─────────────────────────────→│──────────────────────────→│
    │  { authorize: "/authorize", token: "/token", ... }       │
    │←─────────────────────────────│←──────────────────────────│
    │                              │                           │
    │  POST /lisanima/register (RFC 7591)                      │
    │─────────────────────────────→│──────────────────────────→│
    │  { client_id, client_secret }│                           │
    │←─────────────────────────────│←──────────────────────────│
    │                              │                           │
    │  ブラウザを開く               │                           │
    │  GET /lisanima/authorize?client_id=...&code_challenge=...│
    │                              │                           │
    ...
    │←─────────────────────────────│←──────────────────────────│
    │                              │                           │
    │  POST /lisanima/token { code, code_verifier }            │
    │─────────────────────────────→│──────────────────────────→│
...
- stdioモードでは `auth` は無視される（HTTPモード時のみ有効にする制御が必要）

### /auth/pin エンドポイントの追加

`authorize()` からリダイレクトされるPIN入力画面は、FastMCPのルーティング外のカスタムエンドポイント。
Starletteのルートとして追加する:

```python
from starlette.routing import Route
from lisanima.auth.pin import pin_get, pin_post

# FastMCPのASGIアプリにカスタムルートを追加
# 具体的な追加方法は実装時にFastMCPのASGIアプリ構造を確認して決定
```
...
MCPエンドポイントのパスを `/mcp` → `/lisanima` に変更。
OAuthエンドポイントはルートレベルに配置（MCP仕様: MCPサーバーURLからpathを除いたbase URLに配置）。

MCPサーバーURL: `https://quriowork.com/lisanima/mcp`
→ Authorization Base URL: `https://quriowork.com/lisanima`
→ メタデータ: `https://quriowork.com/lisanima/.well-known/oauth-authorization-server`
...
**注意:** OAuthの `/authorize`、`/token`、`/register` 等は全てFastMCPが `127.0.0.1:8765` 上で直接ホストするため、個別のlocationブロックは不要。`/lisanima/` 配下が全てプロキシされる。
