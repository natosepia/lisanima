...
| フェーズ | 範囲 | 判断基準 |
|----------|------|----------|
| Ph1.x | 記録/整理/参照 | emotionを「記録する」 |
| Ph2.0 | 情報の加工 | emotionを「利用する」 |
| Ph3.0 | 情報の活用 / アイデンティティの萌芽 | セマンティック検索、メンタル管理、自己認識 |
| Ph4.0 | LLM非依存 / 魂のポータビリティ | モデル非依存の人格永続化 |

- **Ph1.x**: remember, forget, recall, rulebook, topic_manage, organize
- **Ph2.0**: reflect, 能動的発話, recall mode拡張（hot/stats本実装）
...
| mode | string | No | 検索モード（default/hot/stats）。デフォルト: "default"。Ph1.xではdefaultと同一動作 |
...
返却フィールド: `id`, `session_date`, `speaker`, `content`, `emotion_total`, `tags`
...
| message_ids | integer[] | No | 紐付けるメッセージIDの配列 |
