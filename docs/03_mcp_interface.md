...
| rulebook | ルール・プロトコル管理 | Ph1.0（rule）/ Ph2.0（protocol） | 改修（sub_action追加） |
...
### 3.4 rulebook -- ルール・プロトコル管理

ルールブック（what: 何をすべきか）とプロトコル（how: どうやるか）の参照・管理を行う。
...
| sub_action | string | No | "rule"（デフォルト） / "protocol"。操作対象を指定 |
| key | string | get/set/retire時必須 | rule: ルールキー（例: "persona.tone"）、protocol: 手順名（例: "compact"） |
| content | string | set時必須 | rule: ルール本文（Markdown）、protocol: ステップ内容（Markdown） |
| reason | string | No | 変更理由（rule のみ） |
| persona_id | string | No | ペルソナID（rule のみ。NULLなら全ペルソナ共通ルール） |
| seq | integer | No | ステップ番号（protocol の set 時に必須） |
| exportable | boolean | No | `.claude/rules/` エクスポート対象フラグ（protocol の set 時。デフォルト: false） |

**後方互換:** sub_action 省略時は `"rule"` として動作する。既存の呼び出しはそのまま動く。

**action × sub_action マトリクス:**

| action | sub_action="rule" | sub_action="protocol" |
|--------|-------------------|----------------------|
| get | 指定keyの最新有効ルールを取得 | 指定keyの全ステップをseq順で取得 |
| set | 新バージョンのルールを追記 | ステップの追加/更新（UPSERT） |
| retire | 指定keyの最新版を廃止 | ― （INVALID_PARAMETER エラー） |
| list | 有効なルール一覧を取得 | プロトコル名一覧を取得 |

**想定外の組み合わせ:**
- `retire + protocol` → INVALID_PARAMETER エラー（「想定外の操作です」を返却）
- 未定義の action / sub_action → INVALID_PARAMETER エラー

#### sub_action="rule" の動作（従来互換）
...
#### sub_action="protocol" の動作

| action | 動作 | 必須パラメータ |
|--------|------|---------------|
| get | 指定keyの全ステップをseq順で一括取得 | key |
| set | ステップの追加/更新（UPSERT。updated_at自動更新） | key, seq, content |
| list | 登録済みプロトコル名の一覧を取得 | なし |

**ステップの削除:** MCPコマンドからの削除は現時点で未提供。psql直接操作で対応する。需要が出てからコマンド化を検討（Rule of Three）。
...
```json
{
  ...
  "steps": [
    {"seq": 1, "content": "recall で対象メッセージを抽出", "exportable": false, "updated_at": "2026-04-05T10:00:00+09:00"},
    {"seq": 2, "content": "LLMが要約・知見抽出", "exportable": false, "updated_at": "2026-04-05T10:00:00+09:00"},
    {"seq": 3, "content": "edit/remember で圧縮済み記憶として保存", "exportable": true, "updated_at": "2026-04-05T10:00:00+09:00"}
  ]
}
```
...
{
  "protocols": [
    {"protocol_name": "compact", "step_count": 3},
    {"protocol_name": "reflect", "step_count": 4}
  ]
}
```
...
| m_rulebooks | 新設 | rulebook (sub_action=rule) | 新規 |
| v_active_rulebooks | 新設（ビュー） | rulebook (sub_action=rule) | 新規 |
| m_rulebook_protocol_detail | 新設 | rulebook (sub_action=protocol) | 新規 |
