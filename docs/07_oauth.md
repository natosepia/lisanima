...
DDLの詳細は [04_schema.md](04_schema.md) セクション3.11 / セクション7 を参照。
...
if not fqdn:
    raise RuntimeError("環境変数 LISANIMA_FQDN が未設定です。")
...
- **`issuer_url` はパスなしの `https://{LISANIMA_FQDN}`** を指定する。3/26 auth specではMCPサーバーURLからパスを捨ててAuthorization Base URLを決定するため
- `LISANIMA_FQDN` 環境変数が未設定の場合、`RuntimeError` で起動を中断する
- stdioモードでは `_createMcp()` 内で `auth` 引数を渡さない（FQDNも不要）
...
MCPサーバーURL: `https://<your-domain>/lisanima/mcp`

3/26 auth specではMCPサーバーURLの**パスを捨てて**Authorization Base URLが決定されるため、
OAuthエンドポイント（`/authorize`, `/token`, `/register`）はルートドメインに配置が必要。

→ Authorization Base URL: `https://<your-domain>`（パスなし）
→ ASメタデータ: `https://<your-domain>/.well-known/oauth-authorization-server`
→ PRメタデータ: `https://<your-domain>/.well-known/oauth-protected-resource/lisanima/mcp`
...
```
# FQDN（HTTPモード必須。issuer_url / resource_server_url の生成に使用）
LISANIMA_FQDN=quriowork.com
...
`persona_id` カラム（t_sessions, 将来のidentity等）が拡張余地として機能する。
