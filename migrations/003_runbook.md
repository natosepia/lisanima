...
```bash
pg_dump -U lisa lisanima_db > ~/backup_lisanima_20260317.sql
```

## Step 3: psql接続

```bash
psql -U lisa lisanima_db
```
...
**messagesはここに含めない**（個別SQLで対応するため）。

**期待されるNOTICE:**
- INSERT順が表示される（親→子: t_sessions → t_tags → t_message_tags）
- 各テーブルの移行行数が表示される
- シーケンス設定値が表示される
...
```sql
-- pg_dumpから復元（最終手段）
dropdb -U lisa lisanima_db
createdb -U lisa lisanima_db
psql -U lisa lisanima_db < ~/backup_lisanima_20260317.sql
```
